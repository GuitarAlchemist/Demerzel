...
module Parser =
    ...
    let pIdentifierRaw: Parser<string, unit> =
        ...
        let isRest c = isLetter c || isDigit c || c = '_' || c = '-' || c = '.'
    ...
    let pDocument: Parser<IxQLDocument, unit> =
        wsOrComment >>. many (pTopLevel .>> wsOrComment) .>> eof
        |>> fun items ->
    ...
    let inline private isSuccess (pr: ParserResult<'a, unit>) =
        match pr with
        | CharParsers.Success _ -> true
        | _ -> false

    let private toResult (pr: ParserResult<'a, unit>) : Result<'a, string> =
        match pr with
        | CharParsers.Success(v, _, _) -> Ok v
        | CharParsers.Failure(msg, _, _) -> Result.Error msg

    /// Parse an IxQL string into a document AST
    let parseString (input: string) : Result<IxQLDocument, string> =
        run pDocument input |> toResult
    ...
    let parseStage (input: string) : Result<Stage, string> =
        run (wsOrComment >>. pStage .>> wsOrComment .>> eof) input |> toResult

    /// Parse a single expression from a string
    let parseExpression (input: string) : Result<Expression, string> =
        run (wsOrComment >>. pExpression .>> wsOrComment .>> eof) input |> toResult
