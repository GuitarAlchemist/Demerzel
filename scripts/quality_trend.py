...
def build_rows(evolution_dir: str, out_dir: str, now: datetime) -> list[dict]:
    ...
    for path in sorted(glob.glob(os.path.join(evolution_dir, "*.evolution.json"))):
        ...
        eff_to = (data.get("assessment", {}).get("effectiveness", {})
                  .get("truth_value"))
...
def main() -> int:
    ...
    rows = build_rows(args.evolution_dir, args.out_dir, now)
