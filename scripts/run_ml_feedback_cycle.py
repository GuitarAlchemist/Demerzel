...
--commit closes the loop's LANDING leg, which was a human. The cycle's committed
evidence trail is state/beliefs/*.belief.json (the cycle summaries under
state/oversight/ are gitignored runtime I/O), and ml-governance-schedule.yml goes
RED when that trail goes stale. Until now the refresh reached the repo only if an
operator noticed and committed it by hand: it went 43 days uncommitted (2026-07-30
-> 2026-09-12) while the scheduled task kept succeeding daily, and the one time
before that it landed was also a hand-patch (2875014, "uncommitted in the working
tree ... preserved rather than discarded"). A loop whose output depends on someone
remembering is a loop that dies silently while its producer stays green.

The output is made branch-independent to match the input: --commit refuses to run
anywhere but master, because today's refresh was written onto a feature branch 110
commits behind it, where no amount of committing would have satisfied the guard
that reads master. It fast-forwards BEFORE the cycle runs (master gains commits
daily from demerzel-quality-trend.yml, so a cycle that wrote first would be
unpushable), and it stages only state/beliefs/ so unrelated working-tree edits
never ride along.
...
Exit codes:
  ...
  5  --commit could not guarantee a landing (not on master, or master has
     diverged from origin) — refused BEFORE running the cycle, so a refresh is
     never produced where it cannot land
"""
...
def _sync_master(root: Path, dry: bool) -> dict:
    ...
    and git allows one checkout per branch — that would block `git checkout
