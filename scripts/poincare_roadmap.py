...
# CENTER: Demerzel
ax.plot(0, 0, 'o', color=C['core'], markersize=22, zorder=10)
ax.text(0, 0.04, 'Demerzel', ha='center', va='bottom', fontsize=12, fontweight='bold', color='white')
ax.text(0, -0.04, 'Governance', ha='center', va='top', fontsize=7, color='#8b949e')
...
for label, theta, color in ring1:
    ...
    ax.plot(x, y, 'o', color=color, markersize=15, zorder=8)
    ax.text(x, y + 0.065, label, ha='center', va='bottom', fontsize=6.5, fontweight='bold', color='white')
...
for label, theta, color, status in ring2:
    ...
    ax.plot(x, y, 'o', color=color, markersize=10, zorder=7)
    ax.text(x, y + 0.05, label, ha='center', va='bottom', fontsize=5, color='white')
    ax.text(x, y - 0.04, status, ha='center', va='top', fontsize=4, color=color, fontstyle='italic')
...
for label, theta in ring3:
    ...
    ax.plot(x, y, 'o', color='#8b949e', markersize=6, zorder=6, alpha=0.6)
    ax.text(x, y + 0.04, label, ha='center', va='bottom', fontsize=4, color='#8b949e', alpha=0.8)
...
for i, (label, color) in enumerate(legend):
    y = -1.05 + i * 0.035
    ax.plot(-1.1, y, 's', color=color, markersize=6)
    ax.text(-1.06, y, label, va='center', fontsize=5.5, color='#c9d1d9')

# Depth legend
ax.text(1.05, -0.85, 'Center = Core', fontsize=5, color='#8b949e', ha='right')
ax.text(1.05, -0.89, 'Middle = Active', fontsize=5, color='#8b949e', ha='right')
ax.text(1.05, -0.93, 'Edge = Horizon', fontsize=5, color='#8b949e', ha='right')

# Title
ax.text(0, 1.08, 'GuitarAlchemist Ecosystem Roadmap', ha='center', fontsize=15, fontweight='bold', color='white')
ax.text(0, 1.03, 'Poincare Disk Model — center = core, edge = horizon (infinite distance)', ha='center', fontsize=7, color='#8b949e')

# Stats
stats = '7 repos  |  200+ MCP tools  |  18 grammars  |  24 policies  |  13 departments  |  23 courses  |  6 languages'
ax.text(0, -1.12, stats, ha='center', fontsize=6, color='#58a6ff')
