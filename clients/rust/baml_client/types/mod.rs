// This file was generated by BAML: do not edit it.
// Instead, edit the BAML source files.
//
// Learn more at https://docs.boundaryml.com

//! Generated BAML types.

mod classes;
mod enums;
mod unions;
mod type_aliases;

pub use classes::*;
pub use enums::*;
pub use unions::*;
pub use type_aliases::*;

// Re-export types from baml runtime
pub use baml::{Audio, Image, Pdf, Video};
pub use baml::{Checked, StreamState};
use baml::__internal::serde as serde;

/// All known types in this BAML project.
/// Serves as the compile-time type registry for BamlValue.
#[derive(Debug, Clone, serde::Serialize)]
#[serde(crate = "::baml::__internal::serde", untagged)]
pub enum Types {

    CommandRun(CommandRun),

    HarnessBudget(HarnessBudget),

    HarnessOutputs(HarnessOutputs),

    HarnessResult(HarnessResult),

    QaVerdict(QaVerdict),

    SafetyBounds(SafetyBounds),

    SignalTelemetry(SignalTelemetry),

    SwarmVote(SwarmVote),


    AgentRole(AgentRole),

    AutonomyMode(AutonomyMode),

    BelnapState(BelnapState),

    BudgetTier(BudgetTier),

    CommandStatus(CommandStatus),

    HarnessLane(HarnessLane),


}

impl ::baml::KnownTypes for Types {
    fn as_any(&self) -> &dyn ::std::any::Any {
        self
    }

    fn type_name(&self) -> &'static str {
        match self {

            Self::CommandRun(_) => "CommandRun",

            Self::HarnessBudget(_) => "HarnessBudget",

            Self::HarnessOutputs(_) => "HarnessOutputs",

            Self::HarnessResult(_) => "HarnessResult",

            Self::QaVerdict(_) => "QaVerdict",

            Self::SafetyBounds(_) => "SafetyBounds",

            Self::SignalTelemetry(_) => "SignalTelemetry",

            Self::SwarmVote(_) => "SwarmVote",


            Self::AgentRole(_) => "AgentRole",

            Self::AutonomyMode(_) => "AutonomyMode",

            Self::BelnapState(_) => "BelnapState",

            Self::BudgetTier(_) => "BudgetTier",

            Self::CommandStatus(_) => "CommandStatus",

            Self::HarnessLane(_) => "HarnessLane",



        }
    }
}

impl<'de> serde::Deserialize<'de> for Types {
    fn deserialize<D: serde::Deserializer<'de>>(_deserializer: D) -> ::std::result::Result<Self, D::Error> {
        ::std::result::Result::Err(serde::de::Error::custom("Types is not deserializable, as we cannot disambiguate the type."))
    }
}